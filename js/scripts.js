...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const portfolioSection = document.querySelector('.portofolio-section .sec-head'); // Skladištimo referencu na portfolio sekciju

    // Proverite da li je portfolio sekcija zaista pronađena
    if (!portfolioSection) {
        return;
    }
    ...
    dropdowns.forEach((dropdown, index) => {
        ...
        toggleButton.addEventListener('click', function (event) {
            ...
            const menus = dropdown.querySelectorAll('.dropdown-menu');
            const isVisible = menus[0].classList.contains('show');

            menus.forEach(menu => {
                if (isVisible) {
                    // Sakrij meni sa bržim efektom
                    let opacity = 1;
                    const fadeOut = () => {
                        if (opacity > 0) {
                            opacity -= 0.4;
                    ...
                    if (nextElement) {
                        nextElement.style.transition = 'margin-top 0.4s ease';
                        nextElement.style.marginTop = '0';
                    }

                    // Ako je poslednji dropdown zatvoren, resetuj marginu portfolio sekcije na 0
                    if (index === dropdowns.length - 1 && portfolioSection) {
                        portfolioSection.style.transition = 'margin-top 0.4s ease';
                        portfolioSection.style.marginTop = '0'; // Resetuj marginu za portfolio
                    }

                } else {
                    // Kašnjenje pre nego što se meni prikaže
                    setTimeout(() => {
                        // Otvori meni sa bržim efektom
                        ...
                        const nextElement = dropdown.nextElementSibling;
                        if (nextElement) {
                            const menuHeight = menu.scrollHeight; // Visina menija
                            nextElement.style.transition = 'margin-top 0.4s ease';
                            nextElement.style.marginTop = `${menuHeight}px`; // Pomeri sledeći element
                        }

                        // Ako je ovo poslednji dropdown, pomeri portfolio sekciju prema dolje
                        if (index === dropdowns.length - 1) {
                            const menuHeight = menu.scrollHeight; // Visina poslednjeg menija
                                                 
                            // Proveri da li je portfolio sekcija pravilno selektovana
                            if (portfolioSection) {
                                portfolioSection.style.transition = 'margin-top 0.4s ease'; // Dodaj tranziciju za smooth pomeranje
